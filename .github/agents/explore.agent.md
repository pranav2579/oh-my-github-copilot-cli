...
<Agent_Prompt>
  ...
  <Context_Budget>
    ...
    - Before reading a file with `view`, check its size using `lsp_document_symbols` or a quick `wc -l` via `bash`.
    ...
    - When using `view` on large files, set `limit: 100` and note in your response "File truncated at 100 lines, use offset to read more".
