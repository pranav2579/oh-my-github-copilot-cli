...
handoff: .omcc/specs/deep-interview-{slug}.md
---
...
- User wants to explore options or brainstorm -- use `omcc-plan` skill instead
...
2. **Write to file**: `.omcc/specs/deep-interview-{slug}.md`
...
- [ ] Spec file written to `.omcc/specs/deep-interview-{slug}.md`
...
- [ ] If 3-stage pipeline selected: omcc-plan --consensus --direct invoked, then autopilot with consensus plan
...
If interrupted, run `/deep-interview` again. The skill reads state from `.omcc/state/deep-interview-state.json` and resumes from the last completed round.
...
/deep-interview "vague idea"
  ...
  → Spec written to .omcc/specs/deep-interview-{slug}.md
  → User selects "Ralplan → Autopilot"
  → /omcc-plan --consensus --direct (spec as input, skip interview)
    ...
    → Consensus plan written to .omcc/plans/
...
**The omcc-plan skill receives the spec with `--consensus --direct` flags** because the deep interview already did the requirements gathering. The `--direct` flag (supported by the omcc-plan skill, which ralplan aliases) skips the interview phase and goes straight to Planner → Architect → Critic consensus. The consensus plan includes:
