---
name: cancel
description: Cancel any active OMCC mode (autopilot, ralph, ultrawork, ultraqa, swarm, ultrapilot, pipeline, team)
---

# Cancel Skill

Intelligent cancellation that detects and cancels the active OMCC mode.

**The cancel skill is the standard way to complete and exit any OMCC mode.**
...
**WARNING:** Do NOT use this fallback for `autopilot` or `omcc-teams`. Autopilot requires
`state_write(active=false)` to preserve resume data. omcc-teams requires tmux session
...
REPO_ROOT="$(git rev-parse --show-toplevel 2>/dev/null || { d="$PWD"; while [ "$d" != "/" ] && [ ! -d "$d/.omcc" ]; do d="$(dirname "$d")"; done; echo "$d"; })"
...
elif [ "$REPO_ROOT" != "/" ] && [ -d "$REPO_ROOT/.omcc" ]; then
  OMC_STATE="$REPO_ROOT/.omcc/state"
else
  echo "ERROR: Could not locate .omcc state directory" >&2
  exit 1
fi
...
9. OMCC Teams (tmux CLI workers)
10. Plan Consensus (standalone)
11. Self-Improve (standalone — clear state, clean orphaned worktrees, preserve iteration_state for resume, set status: "user_stopped" in the resolved `<self-improve-root>/state/agent-settings.json`; new runs use `.omcc/self-improve/topics/<topic-slug>/`, with flat `.omcc/self-improve/` retained only for legacy single-track resumes)
...
1. `state_list_active` enumerates `.omcc/state/sessions/{sessionId}/…` to find every known session.
2. `state_clear` runs once per session to drop that session’s files.
3. A global `state_clear` without `session_id` removes legacy files under `.omcc/state/*.json`, `.omcc/state/swarm*.db`, and compatibility artifacts (see list).
4. Team artifacts (`~/.claude/teams/*/`, `~/.claude/tasks/*/`, `.omcc/state/team-state.json`) are best-effort cleared as part of the legacy fallback.
   - Cancel for native team does NOT affect omcc-teams state, and vice versa.

Every `state_clear` command honors the `session_id` argument, so even force mode still uses the session-aware paths first before deleting legacy files.

Legacy compatibility list (removed only under `--force`/`--all`):
- `.omcc/state/autopilot-state.json`
- `.omcc/state/ralph-state.json`
- `.omcc/state/ralph-plan-state.json`
- `.omcc/state/ralph-verification.json`
- `.omcc/state/ultrawork-state.json`
- `.omcc/state/ultraqa-state.json`
- `.omcc/state/swarm.db`
- `.omcc/state/swarm.db-wal`
- `.omcc/state/swarm.db-shm`
- `.omcc/state/swarm-active.marker`
- `.omcc/state/swarm-tasks.db`
- `.omcc/state/ultrapilot-state.json`
- `.omcc/state/ultrapilot-ownership.json`
- `.omcc/state/pipeline-state.json`
- `.omcc/state/omcc-teams-state.json`
- `.omcc/state/plan-consensus.json`
- `.omcc/state/ralplan-state.json`
- `.omcc/state/boulder.json`
- `.omcc/state/hud-state.json`
- `.omcc/state/subagent-tracking.json`
- `.omcc/state/subagent-tracker.lock`
- `.omcc/state/rate-limit-daemon.pid`
- `.omcc/state/rate-limit-daemon.log`
- `.omcc/state/checkpoints/` (directory)
- `.omcc/state/sessions/` (empty directory cleanup after clearing sessions)
...
Report: "No active OMCC modes detected. Use --force to clear all state files anyway."
...
4. In force mode, iterate every active session, call `state_clear` per session, then run a global `state_clear` without `session_id` to drop legacy files (`.omcc/state/*.json`, compatibility artifacts) and report success. Swarm remains a shared SQLite/marker mode outside session scoping.
5. Team artifacts (`~/.claude/teams/*/`, `~/.claude/tasks/*/`, `.omcc/state/team-state.json`) remain best-effort cleanup items invoked during the legacy/global pass.
...
| Force | "All OMCC modes cleared. You are free to start fresh." |
| None | "No active OMCC modes detected." |
...
- **Local-only**: Clears state files in `.omcc/state/` directory
...
1. **Check for active MCP workers**: Look for heartbeat files at `.omcc/state/team-bridge/{team}/*.heartbeat.json`
2. **Send shutdown signals**: Write shutdown signal files for each active worker
3. **Kill tmux sessions**: Run `tmux kill-session -t omcc-team-{team}-{worker}` for each worker
4. **Clean up heartbeat files**: Remove all heartbeat files for the team
5. **Clean up shadow registry**: Remove `.omcc/state/team-mcp-workers.json`

### Force Clear Addition

When `--force` is used, also clean up:
```bash
rm -rf .omcc/state/team-bridge/       # Heartbeat files
rm -f .omcc/state/team-mcp-workers.json  # Shadow registry
# Kill all omcc-team-* tmux sessions
tmux list-sessions -F '#{session_name}' 2>/dev/null | grep '^omcc-team-' | while read s; do tmux kill-session -t "$s" 2>/dev/null; done
```
