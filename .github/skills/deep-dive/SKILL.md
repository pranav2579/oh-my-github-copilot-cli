...
triggers:
  ...
  - "trace and interview"
  - "investigate deeply"
pipeline: [deep-dive, omcc-plan, autopilot]
next-skill: omcc-plan
next-skill-args: --consensus --direct
handoff: .omcc/specs/deep-dive-{slug}.md
---
...
Run the trace autonomously using the `oh-my-github-copilot-cli:trace` skill's behavioral contract.
...
Save to `.omcc/specs/deep-dive-trace-{slug}.md`:
...
After saving:
- Persist `trace_path` in state: `state_write` with `state.trace_path = ".omcc/specs/deep-dive-trace-{slug}.md"`
...
Phase 4 follows the `oh-my-github-copilot-cli:deep-interview` SKILL.md Phases 2-4 (Interview Loop, Challenge Agents, Crystallize Spec) as the base behavioral contract. The executor MUST read the deep-interview SKILL.md to understand the full interview protocol. Deep-dive does NOT duplicate the interview protocol — it specifies exactly **3 initialization overrides**:
...
- Save to `.omcc/specs/deep-dive-{slug}.md`
- Persist `spec_path` in state: `state_write` with `state.spec_path = ".omcc/specs/deep-dive-{slug}.md"`
...
1. **Ralplan → Autopilot (Recommended)**
   ...
   - Action: Invoke `Skill("oh-my-github-copilot-cli:omcc-plan")` with `--consensus --direct` flags and the spec file path (`spec_path` from state) as context. The `--direct` flag skips the omcc-plan skill's interview phase (the deep-dive interview already gathered requirements), while `--consensus` triggers the Planner/Architect/Critic loop. When consensus completes and produces a plan in `.omcc/plans/`, invoke `Skill("oh-my-github-copilot-cli:autopilot")` with the consensus plan as Phase 0+1 output — autopilot skips both Expansion and Planning, starting directly at Phase 2 (Execution).
   - Pipeline: `deep-dive spec → omcc-plan --consensus --direct → autopilot execution`
...
- Use `Agent(subagent_type="oh-my-github-copilot-cli:explore", model="haiku")` for brownfield codebase exploration (Phase 1)
...
- Use `create` tool to save trace result and final spec to `.omcc/specs/`
...
[Phase 5] Spec ready. User selects ralplan → autopilot.
  → omcc-plan --consensus --direct runs on the spec
...
- [ ] Phase 3 saves trace result to `.omcc/specs/deep-dive-trace-{slug}.md` with per-lane critical unknowns
...
- [ ] Final spec saved to `.omcc/specs/deep-dive-{slug}.md` in standard deep-interview format
- [ ] Final spec contains "Trace Findings" section
- [ ] Phase 5 execution bridge passes spec_path explicitly to downstream skills
- [ ] Phase 5 "Ralplan → Autopilot" option explicitly invokes autopilot after omcc-plan consensus completes
...
Deep-dive's output (`.omcc/specs/deep-dive-{slug}.md`) feeds into the standard omc pipeline:

```
/deep-dive "problem"
  → Trace (3 parallel lanes) + Interview (Socratic Q&A)
  → Spec: .omcc/specs/deep-dive-{slug}.md

  → /omcc-plan --consensus --direct (spec as input)
    → Planner/Architect/Critic consensus
    → Plan: .omcc/plans/ralplan-*.md
