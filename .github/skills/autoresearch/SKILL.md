...
- You need durable experiment logs under `.omcc/autoresearch/`
...
<Required_Artifacts>
Canonical persistent storage lives under `.omcc/autoresearch/<mission-slug>/` and/or `.omcc/logs/autoresearch/<run-id>/`.
...
```text
.omcc/autoresearch/<mission-slug>/
  mission.md
  evaluator.json
  runs/<run-id>/
    evaluations/
