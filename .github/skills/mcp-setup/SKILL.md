...
**Options:**
1. **Recommended starter setup** - Fast path for the most common OMCC MCP additions
...
- Run /oh-my-github-copilot-cli:omcc-doctor to diagnose issues

MANAGING MCP SERVERS:
- Add more servers: /oh-my-github-copilot-cli:mcp-setup or `claude mcp add ...`
...
If the custom server is meant to provide organization-specific reference material to OMCC workflows, prefer a single tool named `get_company_context` that returns markdown via `{ context: string }`.
...
Then point OMCC at the full tool name in `.claude/omc.jsonc` or `~/.config/claude-omc/config.jsonc`:
