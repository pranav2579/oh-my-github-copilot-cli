...
2. Scan user skills at `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/omcc-learned/`
3. Scan project skills at `.omcc/skills/`
...
USER SKILLS (~/.claude/skills/omcc-learned/):
| Name              | Triggers           | Quality | Usage | Scope |
|-------------------|--------------------|---------|-------|-------|
| error-handler     | fix, error         | 95%     | 42    | user  |
| api-builder       | api, endpoint      | 88%     | 23    | user  |

PROJECT SKILLS (.omcc/skills/):
| Name              | Triggers           | Quality | Usage | Scope   |
|-------------------|--------------------|---------|-------|---------|
...
5. **Ask for scope:**
   - `user` → `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/omcc-learned/<name>/SKILL.md`
   - `project` → `.omcc/skills/<name>/SKILL.md`
...
✓ Created skill at ~/.claude/skills/omcc-learned/custom-logger/SKILL.md
...
1. **Search for skill** in both scopes:
   - `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/omcc-learned/<name>/SKILL.md`
   - `.omcc/skills/<name>/SKILL.md`
2. **If found:**
   - Display skill info (name, description, scope)
   - **Ask for confirmation:** "Delete '<name>' skill from <scope>? (yes/no)"
3. **If confirmed:**
   - Delete entire skill directory (e.g., `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/omcc-learned/<name>/`)
...
Assistant: Found skill 'old-logger' in user scope:
  ...
  Scope: user (~/.claude/skills/omcc-learned/)
...
2. **Read current content** via `view` tool
...
```
Skill: custom-logger
Scope: user (~/.claude/skills/omcc-learned/custom-logger/)
Description: Enhanced logging with structured output
Triggers: log, logger, logging
Argument Hint: <level> [message]
Quality: 95% (if available)
Usage Count: 42 times (if available)
File Path: /home/user/.claude/skills/omcc-learned/custom-logger/SKILL.md
...
Description: Enhanced logging with structured output
Triggers: log, logger, logging
File: ~/.claude/skills/omcc-learned/custom-logger/SKILL.md
...
**Behavior:**
1. **Scan both scopes:**
   - User skills: `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/omcc-learned/`
   - Project skills: `.omcc/skills/`
...
User: yes
Assistant: ✓ Copied 'error-handler' to .omcc/skills/
...
USER_SKILLS_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/omcc-learned"
...
PROJECT_SKILLS_DIR=".omcc/skills"
...
echo "=== USER-LEVEL SKILLS (~/.claude/skills/omcc-learned/) ==="
if [ -d "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/omcc-learned" ]; then
  USER_COUNT=$(find "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/omcc-learned" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $USER_COUNT"

  if [ $USER_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/omcc-learned" -name "*.md" -type f -exec sh -c '
      FILE="$1"
...
echo ""
echo "=== PROJECT-LEVEL SKILLS (.omcc/skills/) ==="
if [ -d ".omcc/skills" ]; then
  PROJECT_COUNT=$(find ".omcc/skills" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $PROJECT_COUNT"

  if [ $PROJECT_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find ".omcc/skills" -name "*.md" -type f -exec sh -c '
      FILE="$1"
      NAME=$(grep -m1 "^name:" "$FILE" 2>/dev/null | sed "s/name: //")
      DESC=$(grep -m1 "^description:" "$FILE" 2>/dev/null | sed "s/description: //")
      MODIFIED=$(stat -c "%y" "$FILE" 2>/dev/null || stat -f "%Sm" "$FILE" 2>/dev/null)
...
Then ask for scope:
- **User-level** (~/.claude/skills/omcc-learned/) - Available across all projects
- **Project-level** (.omcc/skills/) - Only for this project
...
**Version Control**: Project-level skills (.omcc/skills/) are committed with your code, so the whole team benefits.
...
✓ Project skills directory exists: .omcc/skills/
