...
- **agent-type** - OMCC agent to spawn for the `team-exec` stage (e.g., executor, debugger, designer, codex, gemini). Optional; defaults to stage-aware routing. Use `codex` to spawn Codex CLI workers or `gemini` for Gemini CLI workers (requires respective CLIs installed). See Stage Agent Routing below.
...
The lead writes handoffs to `.omcc/handoffs/<stage-name>.md`.
...
3. **On team cancellation, handoffs survive** in `.omcc/handoffs/` for session resume. They are not deleted by `TeamDelete`.
...
- Extract **agent-type**, validate it maps to a known OMCC subagent
...
Write OMCC state using the `state_write` MCP tool for proper session-scoped persistence:
...
On every stage transition, update OMCC state:
...
4. **Delete team** -- Call `TeamDelete` to clean up:
   ...
   ```
5. **Clean OMCC state** -- Remove `.omcc/state/team-state.json`
...
Task decomposition:
#1 [codex_worker] Security review of current auth code -> output to .omcc/research/auth-security.md
...
|--------|--------------|----------------------|
| **Storage** | JSON files in `~/.claude/teams/` and `~/.claude/tasks/` | SQLite in `.omcc/state/swarm.db` |
...
- **Event log**: All team events are appended to `.omcc/state/team/{teamName}/events.jsonl`
- **Worker status files**: Workers write status to `.omcc/state/team/{teamName}/workers/{name}/status.json`
...
2. OMCC state cleanup via MCP tools:
   ```
...
1. **Worktree creation**: Before spawning a worker, call `createWorkerWorktree(teamName, workerName, repoRoot)` to create an isolated worktree at `.omcc/worktrees/{team}/{worker}` with branch `omcc-team/{teamName}/{workerName}`.
