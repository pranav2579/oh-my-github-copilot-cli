...
`/omcc-teams` is a legacy compatibility skill for the CLI-first runtime: use `omc team ...` commands (not deprecated MCP runtime tools).
...
```bash
/omcc-teams 2:claude "implement auth module with tests"
/omcc-teams 2:codex "review the auth module for security issues"
/omcc-teams 3:gemini "redesign UI components for accessibility"
```
...
- If the task references a plan artifact under one repo (for example
  `tool/.omcc/plans/task-1200-gwd-gifs.md`) and target paths in sibling repos
...
- Do not anchor the launch cwd to only the repo containing `.omcc/plans/...` when
...
- If no safe shared workspace root can be identified, do not launch `/omcc-teams`.
