...
Configure the OMCC HUD (Heads-Up Display) for the statusline.
...
**IMPORTANT**: If the argument is `setup` OR if the HUD script doesn't exist at `~/.claude/hud/omcc-hud.mjs`, you MUST create the HUD files directly using the instructions below.

### Setup Instructions (Run These Commands)

**Step 1:** Check if setup is needed:
```bash
node -e "const p=require('path'),f=require('fs'),d=process.env.CLAUDE_CONFIG_DIR||p.join(require('os').homedir(),'.claude');console.log(f.existsSync(p.join(d,'hud','omcc-hud.mjs'))?'EXISTS':'MISSING')"
```
...
mkdir -p "$HUD_DIR/lib"
cp "${CLAUDE_PLUGIN_ROOT}/scripts/lib/hud-wrapper-template.txt" "$HUD_DIR/omcc-hud.mjs"
...
```bash
node -e "if(process.platform==='win32'){console.log('Skipped (Windows)')}else{require('fs').chmodSync(require('path').join(process.env.CLAUDE_CONFIG_DIR||require('path').join(require('os').homedir(),'.claude'),'hud','omcc-hud.mjs'),0o755);console.log('Done')}"
```
...
```bash
node -e "const p=require('path').join(require('os').homedir(),'.claude','hud','omcc-hud.mjs').split(require('path').sep).join('/');console.log(JSON.stringify(p))"
```
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node ${CLAUDE_CONFIG_DIR:-$HOME/.claude}/hud/omcc-hud.mjs"
  }
}
```
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node C:/Users/username/.claude/hud/omcc-hud.mjs"
  }
}
```
...
```bash
node -e "const p=require('path'),f=require('fs'),d=process.env.CLAUDE_CONFIG_DIR||p.join(require('os').homedir(),'.claude'),t=p.join(d,'hud','omcc-hud.js');try{if(f.existsSync(t)){f.unlinkSync(t);console.log('Removed legacy omcc-hud.js')}else{console.log('No legacy script found')}}catch{}"
```
...
```
[OMCC] ralph | ultrawork | todos:2/5
```

### Focused (Default)
Shows all relevant elements:
```
[OMCC] branch:main | ralph:3/10 | US-002 | ultrawork skill:planner | ctx:67% | agents:2 | bg:3/5 | todos:2/5
```
...
Legacy config location (deprecated): `~/.claude/.omcc/hud-config.json`
...
**Node 24+ compatibility:** The HUD wrapper script imports `homedir` from `node:os` (not `node:path`). If you encounter `SyntaxError: The requested module 'path' does not provide an export named 'homedir'`, re-run the installer to regenerate `omcc-hud.mjs`.

Manual verification:
- HUD script: `~/.claude/hud/omcc-hud.mjs`
