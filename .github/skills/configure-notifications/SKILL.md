...
Set up OMCC notification integrations so you're alerted when sessions end, need input, or complete background tasks.
...
Set up Telegram notifications so OMCC can message you when sessions end, need input, or complete background tasks.

### How This Skill Works

This is an interactive, natural-language configuration skill. Walk the user through setup by asking questions with AskUserQuestion. Write the result to `${CLAUDE_CONFIG_DIR:-~/.claude}/.omcc-config.json`.

### Step 1: Detect Existing Configuration

```bash
CONFIG_FILE="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.omcc-config.json"
...
2. Send /newbot
3. Choose a name (e.g., "My OMCC Notifier")
...
RESPONSE=$(curl -s -w "\n%{http_code}" \
  ...
  -d "text=OMCC test notification - Telegram is configured!")
...
Config saved to: ~/.claude/.omcc-config.json
...
Env vars are auto-detected by the notification system without needing `.omcc-config.json`.

---

## Discord Setup

Set up Discord notifications so OMCC can ping you when sessions end, need input, or complete background tasks.
...
**Options:**
1. **OMCC (default)** - Display as "OMCC"
...
Build the notifications object with the collected values and merge into `.omcc-config.json` using jq:
...
curl -s -o /dev/null -w "%{http_code}" \
  ...
  -d "{\"content\": \"${MENTION:+$MENTION\\n}OMCC test notification - Discord is configured!\"}" \
  "$WEBHOOK_URL"
```
...
Set up Slack notifications so OMCC can message you when sessions end, need input, or complete background tasks.
...
3. Name your app (e.g., "OMCC Notifier") and select your workspace
...
curl -s -o /dev/null -w "%{http_code}" \
  -H "Content-Type: application/json" \
  -d "{\"text\": \"${MENTION_PREFIX}OMCC test notification - Slack is configured!\"}" \
  "$WEBHOOK_URL"
```
...
RAW FIELDS:
  ...
  {{activeMode}}     - Active OMCC mode name
...
curl -X POST \
  ...
  -d '{"event":"test","instruction":"OMCC test notification","timestamp":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' \
  "$WEBHOOK_URL"
```
